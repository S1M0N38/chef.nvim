...
A minimal Neovim plugin is just a `lua/` directory and a `plugin/` autocommand. But a maintainable one needs tests, docs, types, CI, and a release workflow. chef.nvim fills that gap — no framework, no abstraction, just the smallest possible set of opinionated defaults that work.

Starting a plugin shouldn't mean reinventing project structure and CI pipelines from scratch. chef.nvim bundles the conventions used in production plugins so you can focus on writing plugin logic from day one:
...
Get started by reading the comprehensive documentation with [`:help chef`](https://github.com/S1M0N38/chef.nvim/blob/main/doc/chef.txt), which covers all plugin features and configuration options.
